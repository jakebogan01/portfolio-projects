...
{#if showSearch}
    <div class="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24" role="dialog" aria-modal="true" data-headlessui-state="open">
        <div on:keydown|self={()=>{}} on:click|self={()=>{showSearch = false}} class="fixed inset-0 bg-slate-900/25 backdrop-blur transition-opacity opacity-100"></div>
        <div class="relative w-full max-w-lg transform px-4 transition-all opacity-100 scale-100">
            <div class="overflow-hidden rounded-lg bg-white shadow-md" data-headlessui-state="open">
                <div class="relative">
                    <input on:keyup={({ target: { value } }) => debounce(value)} value={search} class="block w-full appearance-none bg-transparent py-4 pl-4 pr-12 text-base text-slate-900 placeholder:text-slate-600 focus:outline-none sm:text-sm sm:leading-6" style="caret-color: rgb(107, 114, 128);" placeholder="Find anything..." aria-label="Search products" role="combobox" type="text" aria-expanded="false" aria-autocomplete="list" data-headlessui-state="" tabindex="0">
