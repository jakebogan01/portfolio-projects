    ...
    <div class="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24" role="dialog" aria-modal="true" data-headlessui-state={showSearch ? 'open' : 'close'}>
        ...
        <div class="relative w-full max-w-lg transform px-4 transition-all opacity-100 scale-100">
            <div class="overflow-hidden rounded-lg bg-white" data-headlessui-state={showSearch ? 'open' : 'close'}>
                <div class="relative">
                    <input on:keyup={({ target: { value } }) => debounce(value)} value={search} class="block w-full appearance-none bg-transparent py-4 pl-4 pr-12 text-base text-slate-900 placeholder:text-slate-600 focus:outline-none sm:text-sm sm:leading-6 border-0 ring-0 ring-transparent focus:ring-transparent focus:outline-0 focus:border-0" style="caret-color: rgb(107, 114, 128);" placeholder="Find anything..." aria-label="Search products" role="combobox" type="text" aria-expanded="false" aria-autocomplete="list" data-headlessui-state="" tabindex="0">
