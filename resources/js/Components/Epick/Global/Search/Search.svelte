...
{#if showSearch}
    <div class="fixed inset-0 flex items-start justify-center pt-16 sm:pt-24" role="dialog" aria-modal="true" data-headlessui-state={showSearch ? 'open' : 'close'} style="z-index: 100;">
        <div transition:fade={{ duration: 200 }} on:keydown|self={()=>{}} on:click|self={hideSearch} class="fixed inset-0 bg-slate-900/25 backdrop-blur transition-opacity opacity-100"></div>
        <div class="relative w-full max-w-lg transform px-4 transition-all opacity-100 scale-100">
            <div transition:fly={{ duration: 300, y: 20, opacity: 0 }} class="overflow-hidden rounded-lg bg-white" data-headlessui-state={showSearch ? 'open' : 'close'}>
                <div class="relative">
