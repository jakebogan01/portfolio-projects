        ...
        <div class="flex items-baseline justify-between border-b border-[#36363b] pb-6 pt-24">
            ...
            <div class="flex items-center">
                <div class="relative inline-block text-left">
                    <div>
                        <button type="button" on:click={()=>{showFilters = !showFilters}} class="inline-flex justify-center text-sm font-medium text-gray-300" id="menu-button" aria-expanded="false" aria-haspopup="true">
                            Sort
                    ...
                    {#if showFilters}
                        ...
                        <div class="absolute right-0 z-10 mt-2 w-40 origin-top-right rounded-md bg-white shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1">
                            <div class="py-1" role="none">
                                ...
                                <a use:inertia="{{href: $page?.url, method: 'get', replace: true, preserveState: true, preserveScroll: true, data: {rating: 'unpopular', price: filters?.price, color: filters?.color, size: filters?.size, gender: filters?.gender, age: filters?.age, style: filters?.style, brand: filters?.brand, shape: filters?.shape}, only: ['products', 'filters']}}" href="#" class="text-gray-500 block px-4 py-2 text-sm" role="menuitem" tabindex="-1" id="menu-item-4">Best Rating</a>
                            </div>
                        </div>
                    {/if}
                </div>
